...
- `SteamWorkshopToolbarController` 当前已接入：
  ...
  - 当前已进一步收口为两套工具栏布局：
    - 总榜模式隐藏“返回总榜”
    - 作者工坊模式隐藏排序 / 时间段 / 筛选，仅保留返回、刷新、缩放、搜索等上下文相关动作
  - 作者工坊模式里，“返回总榜”已调整到右侧自适应分组，跟随登录 / 刷新一起收拢
  - 浏览页分页后的卡片元数据补全已切到 Steam 官方 `GetPublishedFileDetails` 批量接口优先
  - 当前详情补全策略为“列表页抓 ID + 单飞批量详情补全 + UI 渐进回填 + 命中限流退避”
  - `steamcommunity.com/sharedfiles/filedetails` 现在只保留给详情弹层的少量字段兜底，不再承担主链路批量补全
  - 浏览量 / 总订阅 / 总收藏 / 可见性 / 封禁状态 这批字段现在直接以官方接口为准，不再额外用 HTML 重复补全
  - 下一页基础列表预抓后，会温和预热一小批官方详情缓存；真正翻页时优先从详情缓存组装卡片，减少“先半成品后补全”的体感
  - 浏览网格现在会把当前可见卡片 ID 回报给 `SteamWorkshopService`，详情补全队列会优先处理当前视口里的条目，避免快速滚动时中段长期停留在旧状态
  - 作者昵称已补上轻量缓存链：浏览页 / 详情页谁先拿到真实作者名都会记入 SteamWorkshop 缓存，后续同作者条目优先复用，不再反复依赖 HTML 详情页
- 2026-04-02 本轮新增确认：
  - 工具栏筛选菜单已完成中文化
  ...
  - 浏览卡片已接入下载状态：
    ...
    - 下载失败会切到重试态
    - 已下载显示完成态并禁用重复下载
  - 列表滚动到底且确认没有下一页时，底部会显示“没有更多内容了”
  - 翻页提示已改为网格 footer，而不是悬浮 overlay：
    - “正在加载更多项目…”
    - “没有更多内容了”
    - 会跟随卡片网格一起滚动和回弹
  - 详情面板已接入本地状态操作：
