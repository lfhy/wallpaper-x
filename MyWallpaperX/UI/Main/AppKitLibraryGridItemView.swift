...
final class AppKitWallpaperItem: NSCollectionViewItem {
    ...
    private let imageContainer = AppearanceAwareCardContainerView()
    ...
    private func setupViews() {
        ...
        playActionButton.translatesAutoresizingMaskIntoConstraints = false
        playActionButton.isHidden = true
        ...
        imageContainer.addSubview(playActionButton)
        ...
        NSLayoutConstraint.activate([
            ...
            playActionButton.centerXAnchor.constraint(equalTo: imageContainer.centerXAnchor),
            playActionButton.centerYAnchor.constraint(equalTo: imageContainer.centerYAnchor),
            playActionButton.widthAnchor.constraint(equalToConstant: 42),
            playActionButton.heightAnchor.constraint(equalToConstant: 42)
        ])
    ...
    private func ensureCardAnchorCenteredIfNeeded() {
        // 只在 layer 锚点异常时修正，避免每次布局都重复改 frame 导致缩放偏移。
        guard let layer = imageContainer.layer else { return }
        guard !imageContainer.bounds.isEmpty else { return }
        let anchor = layer.anchorPoint
        guard abs(anchor.x - 0.5) > 0.0001 || abs(anchor.y - 0.5) > 0.0001 else { return }
...
private final class AppearanceAwareCardContainerView: NSView {
...
private final class PlayBadgeView: NSView {
    ...
    private func setupView() {
        ...
        glyphView.layer?.shadowRadius = 1.0
        glyphView.layer?.shadowOffset = CGSize(width: 0, height: 0)
    ...
    private func updateGlyphAppearance() {
        ...
        let darkModeBoost: CGFloat = isDarkAppearance ? 0.06 : 0.0
        backdropView.alphaValue = min(1.0, playing ? (1.0 + darkModeBoost) : (0.90 + darkModeBoost))
        layer?.borderColor = NSColor.white.withAlphaComponent(0.50).cgColor
        layer?.shadowOpacity = playing ? 0.18 : 0.16
        glossLayer.opacity = playing ? 1.0 : 0.90

        glyphView.contentTintColor = playing ? .systemRed : .white
        glyphView.layer?.shadowColor = NSColor.black.cgColor
        glyphView.layer?.shadowOpacity = playing ? 0.14 : 0.06
    }
