...
final class AppKitLibraryGridContainerView: NSView {
    ...
    private var boxSelectionMode: BoxSelectionMode?
    private var boxSelectionInitialIDs = Set<String>()
    ...
    private func updateLayoutItemSize() {
        ...
        if flowLayout.itemSize != newSize {
            ...
            // 延迟到当前 layout pass 完成后再 invalidate，避免在 layout() 执行期间
            // 触发子树重新布局，产生 layoutSubtreeIfNeeded 递归警告。
            // 用 animator() 包裹让卡片尺寸/位置变化走动画路径，保留递补动画效果。
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
    ...
    private func calculateColumnCount(width: CGFloat) -> Int {
        ...
        return max(2, min(6, base + wallpaperManager.gridZoomOffset))
    }
    ...
    private func beginBoxSelection(at indexPath: IndexPath?) -> Bool {
        // 框选从初始 selection 快照开始，避免拖拽过程中把中间态直接写成最终态。
        guard wallpaperManager.isMultiSelectMode else { return false }
        let initialIDs = wallpaperManager.selectedWallpaperIds
        boxSelectionInitialIDs = initialIDs

        if let indexPath,
           indexPath.item >= 0,
           indexPath.item < orderedIDs.count {
            let wallpaperID = orderedIDs[indexPath.item]
            boxSelectionMode = initialIDs.contains(wallpaperID) ? .deselect : .select
        } else {
            boxSelectionMode = .select
        }
        return true
    }

    private func updateBoxSelection(in rect: NSRect) {
        // 框选更新只根据初始快照 + 当前碰到的卡片计算最终集合，不做增量手写状态。
        guard let mode = boxSelectionMode else { return }
        let touchedIDs = wallpaperIDs(intersecting: rect)
        var selectedIDs = boxSelectionInitialIDs
        switch mode {
        case .select:
            selectedIDs.formUnion(touchedIDs)
        case .deselect:
            selectedIDs.subtract(touchedIDs)
        }
    ...
    private func endBoxSelection() {
        ...
        boxSelectionInitialIDs.removeAll(keepingCapacity: false)
    }
