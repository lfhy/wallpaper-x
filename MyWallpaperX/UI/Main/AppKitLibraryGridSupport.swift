...
final class AppKitWallpaperCollectionView: NSCollectionView {
...
final class AppKitThumbnailProvider {
    ...
    private let decodeQueue = DispatchQueue(label: "com.mywallpaper.appkit.thumbnail.decode", qos: .utility)
    ...
    func loadThumbnail(for wallpaper: VideoWallpaper, completion: @escaping (NSImage?) -> Void) {
        requestThumbnail(for: wallpaper, completion: completion)
    }

    func prefetchThumbnail(for wallpaper: VideoWallpaper) {
        requestThumbnail(for: wallpaper, completion: nil)
    }
    ...
    private func requestThumbnail(for wallpaper: VideoWallpaper, completion: ((NSImage?) -> Void)?) {
        ...
        lock.lock()
        if inFlight[normalizedPath] != nil {
            if let completion {
                inFlight[normalizedPath]?.append(completion)
            }
            lock.unlock()
            return
        }
        inFlight[normalizedPath] = completion.map { [$0] } ?? []
        lock.unlock()
        ...
        decodeQueue.async { [weak self] in
            ...
            self.finish(normalizedPath: normalizedPath, image: image)
        }
    }

    private func finish(normalizedPath: String, image: NSImage?) {
        lock.lock()
        let completions = inFlight.removeValue(forKey: normalizedPath) ?? []
        lock.unlock()
