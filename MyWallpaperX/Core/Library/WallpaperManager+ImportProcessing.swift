...
extension WallpaperManager {
    ...
    private func scheduleImportedAssetsProcessing(_ urls: [URL]) {
        ...
        importAssetPipelineQueue.async { [weak self] in
            ...
            // 阶段3：静帧完成后补填 fileSize，供「按大小排序」直接读缓存，不再做主线程磁盘 I/O。
            var fileSizeUpdates: [(path: String, size: Int64)] = []
            for url in normalizedURLs {
                guard self.pathExists(url.path) else { continue }
                if let attrs = try? FileManager.default.attributesOfItem(atPath: url.path),
                   let size = attrs[.size] as? Int64 {
                    fileSizeUpdates.append((url.path, size))
                }
            }
            guard !fileSizeUpdates.isEmpty else { return }
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                for update in fileSizeUpdates {
